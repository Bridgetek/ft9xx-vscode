...
function setupExtension(context: vscode.ExtensionContext): void {
    ...
    if ((config as any)['buildTool'] !== 'Ninja' && (config as any)['buildTool'] !== 'MinGW Makefiles') {
        vscode.window.showErrorMessage('Invalid build tool specified in configuration. Reset to default "Ninja"');
        vscode.workspace.getConfiguration().update('ft9xx.buildTool', 'Ninja', vscode.ConfigurationTarget.Global);
    }
