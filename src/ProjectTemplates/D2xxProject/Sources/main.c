...
/* To enable debug, change #undef to #define */
#undef DEBUG
...
int main(void)
{
    ...
    for(;;)
    {
		...
		if (D2XXTEST__Suspend)
		{
			...
			if (SetRemoteWakeup)
			{
				uint8_t wkupSet = SetRemoteWakeup;
				D2XX_IOCTL(0, D2XX_IOCTL_SYS_REMOTE_WAKEUP, &wkupSet, NULL);
				SetRemoteWakeup = 0;
...
#endif
	/* Void reference to retVal to prevent build warnings. */
	(void)retVal;
}
